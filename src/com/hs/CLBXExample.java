...
/**
 * 差旅费报销申请，同步凭证数据到K3Cloud的Action
 * 
 * @author qq
 * 
 */
public class CLBXExample implements Action {

	public String execute(RequestInfo requestInfo) {
		
		InvokeHelper.POST_K3CloudURL = "http://wai.tengchu.com:81/k3cloud/";
		//String dbId = "59c3705927ecfa";//金蝶测试环境账套ID58c794045e98d0
		String dbId = "58c794045e98d0";//金蝶正式环境的账套ID58c794045e98d0
		String uid = "泛微oa凭证接口用户";
		String pwd = "123456";
		int lang = 2052;
		
		new BaseBean().writeLog("**********************差旅费报销申请开始");
		...
		// *********************************************************************************************************************
		new BaseBean().writeLog("**********************处理主表数据开始");
		...
		rs1.executeSql("select lastoperatedate from workflow_requestbase where requestId='"
				+ requestid + "'");
		log.writeLog("查看最后操作日期的SQL语句：" + sql);
		
		if(rs1.next()){
			new BaseBean().writeLog("111111");
			FDate=rs1.getString("lastoperatedate"); 
			new BaseBean().writeLog("最后操作日期：" + FDate);//2017-09-21
		
			//会计年度 
			FYEAR=FDate.substring(0, 4);
			new BaseBean().writeLog("会计年度：" +FYEAR);
		  
			//会计期间 
			String[] str = FDate.split("-"); FPERIOD = str[1]; 
			new BaseBean().writeLog("会计期间：" + FPERIOD); 
		}
		...
		for (int i = 0; i < properties.length; i++) {
			...
			if (name.equals("hjje")) {
				...
				log.writeLog("对象中存储的贷方金额合计为："+hjje);
			}
		...
		if (sqr != null && !"".equals(sqr)) {
			...
			if (rs.next()) {
				...
				log.writeLog("申请人名称：" + lastname);
			}
		}
		
		// *********************************************************************************************************************
		new BaseBean().writeLog("**********************调用K3Cloud接口开始");
		
		
		// 明细表1
		log.writeLog("进入处理明细数据");
		DetailTable detailTable0 = requestInfo.getDetailTableInfo().getDetailTable(0);
		int length1 = detailTable0.getRowCount();
		log.writeLog("length1:"+length1);
...
//**************************************处理拼接明细表数据开始**************************************************************
				...
				log.writeLog("length2:"+length2);
				
				int length = length1 > length2 ? length1 : length2;// 获取最大的明细行
				log.writeLog("length:"+length);
				for (int i = 0; i < length; i++) {
					...
					if(length1>i){
						...
						for (int a = 0; a < detailTable0.getRow()[i].getCell().length; a++) {
							...
							if (name.equals("yskm")) {// 预算科目
								...
								log.writeLog("查看OA的预算科目的SQL语句："+rsoakm);
								if (oakm.next()) {
									codename = oakm.getString("codename");
									log.writeLog("OA科目编码：" + codename);
								}
							...
							if (name.equals("je")) {//金额(借方金额)
								...
								log.writeLog("借方金额为："+je);
								
								// 计算借方总额
								debitTotal += je;
								log.writeLog("对象中存储的借方总金额为："+debitTotal);
							...
							if (name.equals("cdzt")) {// 承担主体部门
								...
								if (oacbzx.next()) {
									...
									log.writeLog("查看金蝶的预算科目的SQL语句："+sqlkm);	
									...
									if (rskm.next()) {
										...
										log.writeLog("OA科目名称：" + oayskm);
										log.writeLog("科目名称：" + FAccountName);
										log.writeLog("科目编码：" + FAccountId);
										log.writeLog("部门编码：" + FDetailID);
										log.writeLog("品牌推广项目：" + pptgxm);
										log.writeLog("客户推广项目：" + khtgxm);
										log.writeLog("客户开发项目：" + khkfxm);
										log.writeLog("自主研发项目：" + zzyfxm);
										log.writeLog("行业分类：" + xyfl);
									}
									
								}
								FEXPLANATION=requestname+":"+oayskm+lcbh;
								log.writeLog("摘要：" + FEXPLANATION);
					...
					if(length2<(i+1)){
						...
						// 循环第三个明细表
						log.writeLog("第三个明细表数组："+ detailTable2.getRow());
						
							for (int a = 0; a < detailTable2.getRow()[i].getCell().length; a++) {
								//log.writeLog("第"+i+"次进入明细表31111");
								Cell cell = detailTable2.getRow()[i].getCell()[a];
								String name = cell.getName();// 明细字段名称
								String value = cell.getValue();// 明细字段的值
								//log.writeLog("进入第三个明细表第二个循环");
								log.writeLog("第三个明细表name:"+name);
								if (name.equals("skje")) {//合计金额（收款金额的合计）
									/**
									 * 贷方金额
									 */
									//log.writeLog("第"+i+"次进入明细表32222");
									skje= Double.parseDouble(value.trim());
									log.writeLog("贷方金额为："+skje);
									
									// 计算贷方总额
									creditTotal += skje;
									log.writeLog("对象中存储的贷方总金额为："+creditTotal);	
									
									/**
									 * 原币金额【(单据体)原币金额=(单据体)借方金额+(单据体)贷方金额】
									 */
									FAMOUNTFOR=je+skje ;
									log.writeLog("对象中存储的原币金额为："+FAMOUNTFOR);
									
									/**
									 * 本位币金额
									 */
									FAMOUNT =FAMOUNTFOR;
									log.writeLog("对象中存储的本位币金额为："+FAMOUNT);
				...
				log.writeLog("数据内容sContent："+sContent);
				
				try {
					InvokeHelper.Save(sFormId, sContent);
				} catch (Exception e) {
					e.printStackTrace();
					log.writeLog("调用保存接口报错信息：" + e.getMessage());
				}

				log.writeLog("hola success");
			}
		} catch (Exception e) {
			e.printStackTrace();
			log.writeLog("调用登录接口报错信息：" + e.getMessage());
		}
